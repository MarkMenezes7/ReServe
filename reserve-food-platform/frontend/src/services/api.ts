...
export const mlApi = {
  ...
  getHealth: () => apiRequest<{ status: string }>('/api/ml/health'),
  getPeakHours: () => apiRequest<{ hours: { hour: number; count: number }[] }>('/api/ml/analytics/peak-hours'),
  getCategoryTrends: () => apiRequest<{ trends: Record<string, unknown>[] }>('/api/ml/analytics/category-trends'),
  getDonorPatterns: () => apiRequest<{ donors: Record<string, unknown>[] }>('/api/ml/analytics/donor-patterns'),
};
